...
/**
 * Compile a String or other {@link CharSequence}, returning a Java {@link Class} instance that may be instantiated. This class is a Facade around {@link JavaCompiler} for a
 * narrower use case, but a bit easier to use.
 * <p>
 ...
 * </pre>
 * 
 * The source can be in a String, {@link StringBuffer}, or your own class which implements {@link CharSequence}. If you implement your own, it must be thread safe (preferably,
 * immutable.)
 * 
...
public class CharSequenceCompiler<T> {
	...
	public CharSequenceCompiler(final ClassLoader loader, final Iterable<String> options) {
		...
		// create our FileManager which chains to the default file extensionManager
		// and our ClassLoader
	 ...
	 * 
	 * @param classes
	 *            A Map whose keys are qualified class names and whose values are the Java source strings containing the definition of the class. A map value may be null,
	 *            indicating that compiled class is expected, although no source exists for it (it may be a non-public class contained in one of the other strings.)
	 * @param diagnosticsList
	 *            Any diagnostics generated by compiling the source are added to this list.
	 * @return A mapping of qualified class names to their corresponding classes. The map has the same keys as the input <var>classes</var>; the values are the corresponding Class
	 *         objects.
...
/**
 * A JavaFileManager which manages Java source and classes. This FileManager delegates to the JavaFileManager and the ClassLoaderImpl provided in the constructor. The sources are
 * all in memory CharSequence instances and the classes are all in memory byte arrays.
 */
final class FileManagerImpl extends ForwardingJavaFileManager<JavaFileManager> {
	...
	/**
	 * @return the class loader which this file extensionManager delegates to
	 */
	public ClassLoader getClassLoader() {
 ...
 * </ol>
 * This class does not attempt to reuse instances (there does not seem to be a need, as it would require adding a Map for the purpose, and this would also prevent garbage
 * collection of class byte code.)
 */
